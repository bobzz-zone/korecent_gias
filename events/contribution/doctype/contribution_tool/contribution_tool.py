...
class ContributionTool(Document):
	def calculate(self):
		...
		items = frappe.db.sql("""select item_code, sum(qty),sum(amount) from `tabSales Invoice Item` where parent IN ({}) group by item_code """.format(invoice),as_list=1)
		if devided=="Qty":
			for row in items:
				total_qty+=flt(row[1])
		self.result=[]
